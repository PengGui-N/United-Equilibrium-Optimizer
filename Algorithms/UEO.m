...
%function [Ceq1_fit,Ceq1,Convergence_curve,runtime] = EO(fobj, dim, constraints, Particles_no, Max_iter)
...
%Convergence_position=ones(Max_iter,dim);
...
while 1
for united=1:dim
C_united(united)=C_un(ceil(rand*3),united);
end
    tf = isequal(C_united,Ceq1)+isequal(C_united,Ceq2)+isequal(C_united,Ceq3);
        if tf==0
            break; 
        end
end
%Ceq_ave=(Ceq1+Ceq2+Ceq3+Ceq4)/4;       
Ceq_ave2=(Ceq1+Ceq2+Ceq3)/3;  
Ceq_ave3=(Ceq1+Ceq2)/2;  % averaged candidate 
Ceq_ave4=(Ceq1+Ceq2+Ceq3+C_united)/4;   
C_pool=[Ceq1;Ceq2;Ceq3;Ceq_ave2;Ceq_ave3;Ceq_ave4];                     % Equilibrium pool
 ...
 t=(1-Iter/Max_iter)^(a2*Iter/Max_iter);                      % Eq (9)
    ...
    for i=1:Particles_no
           ...
           F=ceil(rand*3)*sign(r-rand).*(exp(-lambda.*t)-1);             % Eq(11)
           r1=rand(); r2=rand();                              % r1 and r2 in Eq(15)
           GCP=ceil(rand*2)*r1*ones(1,dim)*(r2>=GP);                   % Eq(15)
          % GCP=ceil(rand*2)*r1*ones(1,dim);  
